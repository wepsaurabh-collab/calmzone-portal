...
async function getToken() {
    ...
    try {
        ...
        if(r.data && r.data.api_token) { 
            tokenCache = r.data.api_token; tokenExp = Date.now() + 3000000; return tokenCache; 
        }
        throw new Error("Invalid Token");
    } catch(e) { throw new Error("API Authentication Failed. Check API Key or IP Whitelist."); }
}
...
// FIXED BOOKING PAYLOAD AS PER iCARRY PDF
app.post('/api/book', async (req, res) => {
    const { pin, courier_name, price, weight, pickup_id, consignee, parcel } = req.body;
    ...
    try {
        ...
        // Strictly matching the PDF JSON Array format
        const icarryPayload = {
            ...
            "consignee[state]": consignee.state, // Ensure 2 char state code
            ...
            "parcel[weight][weight]": parcel.wt || "500", 
            "parcel[weight][unit]": "gm", 
            "parcel[dimensions][length]": parcel.l || "10", 
            "parcel[dimensions][breadth]": parcel.b || "10", 
            "parcel[dimensions][height]": parcel.h || "10", 
            "parcel[dimensions][unit]": "cm"
        };

        const liveRes = await axios.post(`${ICARRY_URL}/api_add_shipment_surface&api_token=${token}`, icarryPayload);

        if(liveRes.data && liveRes.data.awb) { 
            awb = liveRes.data.awb;
            pdf = "https://www.w3.org/WAI/ER/tests/xhtml/testfiles/resources/pdf/dummy.pdf";
        } else {
             throw new Error(JSON.stringify(liveRes.data) || "iCarry rejected the booking payload.");
        }
        ...
        res.status(500).json({ status: "FAILED", msg: "iCarry API Error: " + err.message });
    }
});
...
app.post('/api/pay-due', async (req, res) => {
    ...
    if(u.balance < s.disc_amt) return res.status(400).json({ status:"FAILED", msg:"Low balance!" });
